...
resource "cloudflare_api_token" "this" {
  name = "k8s.${var.cluster_name}.${var.identifier}.acme"
...
resource "kubernetes_secret" "cf_token" {
  metadata {
    name      = "${var.identifier}.cloudflare-token"
...
resource "kubernetes_manifest" "issuer" {
  manifest = {
    ...
    kind       = "ClusterIssuer"
    metadata   = { name = "${var.identifier}.cluster-issuer" }
    spec = {
      acme = {
        email               = var.acme.email
        privateKeySecretRef = { name = "${var.identifier}.account-key" }
